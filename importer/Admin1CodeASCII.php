...
/**
 * Класс для скачивания и импорта регионов из http://download.geonames.org/export/dump/
 *
 * В качестве адреса, по которому расположена БД стран используется URL CSV-файла (по умолчанию http://download.geonames.org/export/dump/admin1CodesASCII.txt).
 * Хранится в переменной $downloadUrl.
 *
 * Пример использования:
 * ```php
 * $importer = new admin1CodeASCII('/tmp/', function($region) {
...
class Admin1CodeASCII
{
    ...
    /**
     * @var string базовый URL для скачивания регионов в формате CSV
     */
    public $downloadUrl = 'http://download.geonames.org/export/dump/admin1CodesASCII.txt';
     ...
     * ```php
     * function($region) {
     *     print 'Consume region: ' . print_r($region) . "\n";
     *     // do something else
     * }
     * ```
    ...
    protected function consumeRow($row)
    {
        if (!empty($row) && !empty($this->columns) && count($row) == count($this->columns)) {
            $region = array_combine($this->columns, $row);
            call_user_func_array($this->rowConsumer, [$region]);
        }
    }
